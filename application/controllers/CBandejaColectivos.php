...
class CBandejaColectivos extends CI_Controller {
	...
	public function ajax_colectivos(){
		...
		$perfiles_asistenciales = $this->MBandejaColectivos->getProfileByGroup(2);
	...
	public function cambio_bandeja(){
		...
		$datos_tweet = $this->MBandejaColectivos->obtenerTweet($id_tweet);
		...
		if($insert){
			...
			// Actualizamos el status del tweet en la tabla 'bandeja_colectivos'
			$data2 = array(
				'id_str' => $id_tweet,
			...
			$update = $this->MBandejaEntrada->update_status('bandeja_colectivos', $data2);
