$(document).ready(function(){
	...
	var tabOperantes = $('#tab_operantes').DataTable({
        //~ "paging": true,
        //~ "lengthChange": false,
        "autoWidth": false,
        //~ "searching": true,
	...
	$("table#tab_operantes").on('change', 'select.cambiar', function (e) {
    ...
    // Función para cambiar un twitter a la bandeja de respuestas, asignado al perfil indicado
	$("table#tab_operantes").on('click', 'a.observacion', function (e) {
	...
	$("#asignar").on('click', function (e) {
		...
		if($("#detalles").val().trim() == ''){
			...
			$.post(base_url+'operantes/cambiar_bandeja', {'id':$("#id_tweet").val(), 'nueva_bandeja':$("#nueva_bandeja").val(), 'mensaje':$("#detalles").val(), 'id_perfil':$("#id_perfil").val(), 'get_ids':$("#get_ids").val()}, function (response) {
				...
				if (response['response'] == "error") {
					...
					function(){
					 ...
					 swal({ 
					   ...
					   function(){
						 window.location.href = base_url+'bandeja_operantes';
					 });
				}
			}, 'json');
	...
	$("table#tab_operantes").on('click', 'a.verId', function (e) {
	...
	$("table#tab_operantes").on('click', 'a.verName', function (e) {
	...
	// Función para ver el time-line de un twitter tomando en cuenta el valor del id
	$("table#tab_operantes").on('click', 'a.verText', function (e) {
